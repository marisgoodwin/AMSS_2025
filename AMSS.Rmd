...
metabarcoding complement conventional fisheries approaches (e.g., netting) by providing in-depth, rapid, and non-invasive assessments of species composition. We implemented eDNA metabarcoding to assess its utility as an ecological monitoring tool in 5 estuarine sites in Kachemak Bay, Alaska with varying degrees of glacial coverage (0 – 60%). We analyzed (n=100) eDNA samples from April and September 2022 in Kachemak Bay to tease apart
...
kb_seine$SampleID <- paste(kb_seine$SiteAcronym, kb_seine$SamplingPeriod,kb_seine$SeineSetNumber,sep = "_")
...
print(asymptotic_estimates)
# Interpretation: High Sample Coverage (SC = 0.9408): This suggests that the seine sampling effort has captured about 94.08% of the species diversity in the assemblage. While this is quite high, it also indicates that there are still some species that might be discovered with additional sampling.
```
...
unique_taxa <- unique_taxa %>% 
  inner_join(seine_taxa, by = c("ScientificName" = "sheetname"))
# add in seine as the method
unique_taxa$Method <- "seine"
...
nmds_coords$SampleID <- rownames(nmds_coords)# I had to do some fenagling for this, it will not run on its own until I fix the code above from deleting the sampleID column/rownames of the site_by species matrix
...
# eDNA data analysis. We are going to follow Zack's decontam protocol.
...
# we have 103 samples, 1257 hashes, and 6832925 reads starting out
```
...
# now we have 95 samples, 669 hashes, and 4651142 reads 
```
...
# now we have 86 samples, 667 hashes, and 4649397 reads 
...
# we still have 86 samples, 667 hashes, and 4649397 reads 
...
```
make the pairwise distances a long table. NOTE: THIS IS NOT IDENTIFYING SAMPLES CORRECTLY. I THINK WE ARE GOING TO SKIP THIS FOR NOW BECAUSE THE DATA IS WHACK! 
```{r}
...
### WE ARE SKIPPING DISSIMILARITY BETWEEN PCR REPLICATES BECAUSE THE LINES ABOVE ARE NOT IDENTIFYING SAMPLES CORRECTLY. 
...
ASV_nested %>% 
  dplyr::select(Step2.tibble) %>% 
  unnest(Step2.tibble,names_sep = "_") %>% 
  mutate(Step2.tibble_nReads = as.integer(Step2.tibble_nReads)) %>% write_csv("~/Documents/GITHUB/AMSS_2025/outputs/ASV_table_after_cleanup.csv")
ASV_nested %>% 
  dplyr::select(Step2.tibble) %>% 
  unnest(Step2.tibble,names_sep = "_") %>% 
  distinct(Step2.tibble_Hash) %>% 
  left_join(Hash_key_blast, by = c("Step2.tibble_Hash" = "Hash")) %>% 
  write_csv("~/Documents/GITHUB/AMSS_2025/outputs/Hash_Key_after_cleanup.csv")
```

```{r}
# read in the updated data 
kb_ASV_update <- read_csv("/Users/marisgoodwin/Documents/GITHUB/AMSS_2025/outputs/ASV_table_after_cleanup.csv")
Hash_key_update <- read_csv("/Users/marisgoodwin/Documents/GITHUB/AMSS_2025/outputs/Hash_Key_after_cleanup.csv")

# change the column names to remove the Step2.tibble_ prefix
colnames(kb_ASV_update) <- gsub("Step2.tibble_", "", colnames(kb_ASV_update))
# now the same for the Hash key column called Step2.tibble_Hash
colnames(Hash_key_update) <- gsub("Step2.tibble_", "", colnames(Hash_key_update))

# now we can join the ASV table with the Hash key
kb_ASV_update %>%
  left_join(Hash_key_update, by = c("Hash" = "Hash")) -> kb_ASV_table
...
site_by_species_eDNA <- kb_ASV_table %>%
  dplyr::select(sample_id, sum_taxa, Count) %>%
  distinct() %>%
  spread(key = sum_taxa, value = Count, fill = 0)

edna_sampleID <- site_by_species_eDNA$sample_id
...
# Seine Assemblage: With a sample coverage of 94.08%, the seine observed most of the species, but additional sampling could have revealed more species. The majority of species are found in only one or two sites, indicating a relatively high level of site-specific species.

# on the other hand, the eDNA assemblage has a sample coverage of 98.75%, indicating that nearly all the species diversity has been captured. The species are more evenly distributed across the sites, with several species found in multiple sites, indicating a more homogeneous distribution. 
...
```{r}
# I need to make sure the sampling month is included in the kb_ASV_table
kb_ASV_table <- kb_ASV_table %>%
  mutate(SamplingMonth = case_when(
    grepl("422", sample_id) ~ "April",
    grepl("922", sample_id) ~ "September",
    TRUE ~ "NA"
  ))

# Filter and select unique taxa for April and September with Count > 0
april_taxa_edna <- kb_ASV_table %>%
  filter(grepl("April", SamplingMonth) & Count > 0) %>%
  dplyr::select(sum_taxa) %>%
  distinct() %>%
  pull(sum_taxa)

september_taxa_edna <- kb_ASV_table %>%
  filter(grepl("September", SamplingMonth) & Count > 0) %>%
  dplyr::select(sum_taxa) %>%
  distinct() %>%
  pull(sum_taxa)
...
# having talked to peter and chris guo, it doesn't seem like clarkii, cylindraceum, and mordax would be found in kbay. let's look at what samples these occured in for april.
kb_ASV_table %>%
  filter(sum_taxa %in% c("Eukaryota,Chordata,Actinopteri,Salmoniformes,Salmonidae,Oncorhynchus,Oncorhynchus clarkii", "Eukaryota,Chordata,Actinopteri,Salmoniformes,Salmonidae,Prosopium,Prosopium cylindraceum", "Eukaryota,Chordata,Actinopteri,Osmeriformes,Osmeridae,Osmerus,Osmerus mordax") & grepl("April", SamplingMonth)) %>%
  dplyr::select(sample_id, sum_taxa, Count) %>%
  arrange(sum_taxa, desc(Count))
# each of these only occured in one PCR replicate and a single sample for each, at a different site. I think we can safely remove these from the dataset.
kb_ASV_table <- kb_ASV_table %>%
  filter(!sum_taxa %in% c("Eukaryota,Chordata,Actinopteri,Salmoniformes,Salmonidae,Oncorhynchus,Oncorhynchus clarkii", "Eukaryota,Chordata,Actinopteri,Salmoniformes,Salmonidae,Prosopium,Prosopium cylindraceum", "Eukaryota,Chordata,Actinopteri,Osmeriformes,Osmeridae,Osmerus,Osmerus mordax"))
...
for (i in 1:nrow(kb_ASV_table)){
  if (!is.na(kb_ASV_table$species[i])){
    kb_ASV_table$TaxonomicLevel[i] <- "Species"
  } else if (!is.na(kb_ASV_table$genus[i])){
    kb_ASV_table$TaxonomicLevel[i] <- "Genus"
  } else if (!is.na(kb_ASV_table$family[i])){
    kb_ASV_table$TaxonomicLevel[i] <- "Family"
  } else if (!is.na(kb_ASV_table$order[i])){
    kb_ASV_table$TaxonomicLevel[i] <- "Order"
  } else if (!is.na(kb_ASV_table$class[i])){
    kb_ASV_table$TaxonomicLevel[i] <- "Class"
  } else if (!is.na(kb_ASV_table$phylum[i])){
...
taxonomic_counts <- kb_ASV_table %>%
  group_by(TaxonomicLevel, sum_taxa, nReads > 0) %>%
  summarise(n = n()) %>%
  distinct()
...
# 28 identified to species, 8 to genus, and 3 to family.

```
Now compare eDNA and seine 
```{r}
# Extract unique taxa from the seine data
seine_taxa <- unique_taxa

# Extract unique taxa from the eDNA data for April and September with Count > 0
edna_taxa <- kb_ASV_table %>%
  filter((grepl("April", SamplingMonth) | grepl("September", SamplingMonth)) & Count > 0) %>%
  dplyr::select(sum_taxa) %>%
  distinct() %>%
  pull(sum_taxa)
# let's separate the sum_taxa into the different taxonomic levels. we will just join with the kb_ASV_table to get the taxonomic levels
edna_taxa <- kb_ASV_table %>%
  filter(sum_taxa %in% edna_taxa) %>%
  dplyr::select(sum_taxa, species, genus, family, order, class, phylum) %>%
  distinct()

# Create a list of the taxa
taxa_list <- list(Seine = seine_taxa, eDNA = edna_taxa)
# each of these lists has taxa broken down by taxonomic level. seine has the following columns: ScientificName,Phylum,Class,Order,Family,Genus,Species, and Method. eDNA has the following columns: sum_taxa, species, genus, family, order, class, and phylum. we will only compare the sum_taxa column from eDNA to the ScientificName column from seine. I want to create an upset plot to compare at each of these taxonomic levels.

# create a list of the taxa split by taxonomic level
taxa_species <- list(Seine = seine_taxa$ScientificName, eDNA = edna_taxa$species)
taxa_genus <- list(Seine = seine_taxa$Genus, eDNA = edna_taxa$genus)

# Create the Venn diagram with labels
venn_plot <- ggVennDiagram(taxa_species, label_alpha = 0) +
  ggtitle("Venn Diagram of Unique species in Seine and eDNA Samples") +
  theme(legend.position = "none") # Remove the legend for clarity
print(venn_plot)
unique_seine <- setdiff(taxa_species$Seine, taxa_species$eDNA)
unique_edna <- setdiff(taxa_species$eDNA, taxa_species$Seine)
shared_taxa <- intersect(taxa_species$Seine, taxa_species$eDNA)
venn_genus <- ggVennDiagram(taxa_genus, label_alpha = 0) +
  ggtitle("Venn Diagram of Unique genus in Seine and eDNA Samples") +
  theme(legend.position = "none") # Remove the legend for clarity
print(venn_genus)
unique_seine <- setdiff(taxa_genus$Seine, taxa_genus$eDNA)
unique_edna <- setdiff(taxa_genus$eDNA, taxa_genus$Seine)
shared_taxa <- intersect(taxa_genus$Seine, taxa_genus$eDNA)
# create a df with unique_seine, unique_edna, and shared_taxa

##### stopped here jan 18th 4:30PM
...
##############################################
This is code that I am just messing around with. Other ways that people clean their data before going into analyses. The code above is from Zack's gruinard decon package. 

This is code to help me figure out microDecon
```{r}
