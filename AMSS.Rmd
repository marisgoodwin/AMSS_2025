...
# we have 196 samples, 1735 hashes, and 16230549 reads starting out
```
...
# now we have 196 samples, 1609 hashes, and 9860950 reads 
```
...
# now we have 187 samples, 1603 hashes, and 9859823 reads 
...
# we still have 187 samples, 1603 hashes, and 9859823 reads 
...
# remove the pc and technc samples, and nc
kb_ASV_table <- kb_ASV_table %>%
  filter(!grepl("pc|technc|NC|nc", sample_id))
unique(kb_ASV_table$sample_id)
```

```{r}
kb_ASV_table <- kb_ASV_table %>%
  group_by(sample_id, sum_taxa.x) %>%
  mutate(Count = sum(nReads)) %>%
  ungroup() %>%
  distinct()

site_by_species_eDNA <- kb_ASV_table %>%
  dplyr::select(sample_id, sum_taxa.x, Count) %>%
  distinct() %>%
...
# on the other hand, the eDNA assemblage has a sample coverage of 99.16%, indicating that nearly all the species diversity has been captured. The species are more evenly distributed across the sites, with several species found in multiple sites, indicating a more homogeneous distribution. 
...
venn_species <- ggVennDiagram(taxa_list, label_alpha = 0) +
...
kb_ASV_table <- kb_ASV_table %>%
  filter(!sum_taxa.x %in% c("Eukaryota,Chordata,Actinopteri,Salmoniformes,Salmonidae,Oncorhynchus,Oncorhynchus clarkii", "Eukaryota,Chordata,Actinopteri,Salmoniformes,Salmonidae,Prosopium,Prosopium cylindraceum", "Eukaryota,Chordata,Actinopteri,Osmeriformes,Osmeridae,Osmerus,Osmerus mordax"))
...
edna_taxa <- kb_ASV_table %>%
  filter(sum_taxa.x %in% edna_taxa) %>%
  dplyr::select(sum_taxa.x, species.x, genus.x, family.x, order.x, class.x, phylum.x) %>%
  distinct()
